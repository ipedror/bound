...
import { Line, Shape as KonvaShape, Transformer, Group } from 'react-konva';
import type Konva from 'konva';
import type { Shape } from '../../types/shape';
import type { Position } from '../../types/base';
import { SELECTION_COLOR } from '../../constants/canvas';
import { drawRoughLine, isHandDrawn, seedFromString } from '../../utils/canvas/roughDraw';
...
export const LineShape: React.FC<LineShapeProps> = React.memo(
  ({ shape, isSelected, isDraggable, onSelect, onUpdate, onDragStart, onDragEnd }) => {
    const shapeRef = useRef<Konva.Line | Konva.Shape>(null);
    ...
    const handleDragEnd = useCallback(
      (e: Konva.KonvaEventObject<DragEvent>) => {
        ...
        // Normal Line uses x/y as drag delta (starts at 0,0)
        onUpdate({
    ...
    const sceneFunc = useMemo(() => {
      ...
      return (context: Konva.Context) => {
        ...
        ctx.save();
        // Offset points relative to shape position
